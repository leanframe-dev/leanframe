...
def test_nested_column_access():
    ...
    result = extracted_df.assign(age_group="adult", has_email=True)
    expected_columns_after_assign = [
        "id",
        "person_name",
        "person_age",
        "email",
    ...
    try:
        pandas_result = (
            lf_df._data.to_pyarrow().read_all().to_pandas(types_mapper=pd.ArrowDtype)
        )
...
def test_pandas_conversion_fallback():
    ...
    ibis_table = lf_df._data
    extracted_table = ibis_table.select(
        ibis_table.id, ibis_table["person"]["name"].name("person_name")
    )
